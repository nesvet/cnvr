...
export const validExtensions = [ "js", "cjs", "mjs" ];
...
export class Entrypoints extends Map {
	constructor({ entrypointsWatchQueueDelay } = {}, immediate) {
		...
		const conveyerFileNames = Entrypoints.getConveyerFiles(process.argv.slice(2).filter(arg => !/^-/.test(arg)));
			...
			for (let fileName of args) {
				...
				if (stats.isFile())
					...
					for (const anotherFileName of fs.readdirSync(dirName))
						if (/^\.conveyer\.[cm]?js$/.test(anotherFileName)) {
