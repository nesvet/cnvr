...
import { local, Packages } from "#utils";
...
export class ESBuild extends Stage {
	constructor({
		...
		super({
			...
			color: true,
			...localOption && {
				local: Array.isArray(localOption) ? localOption : [ localOption ]
			},
			...restOptions,
	...
	async handleInited() {
		...
		let external;
		if (this.external) {
			const localMatcher = this.local && anymatch(this.local);
			external = (
				this.external === true ?
					this.conveyerContext.packages.external.asNames() :
					this.external.includes(true) ?
						this.external.flatMap(packageName => packageName === true ? this.conveyerContext.packages.external.asNames() : packageName) :
						this.external
			).filter(localMatcher ? testString => !localMatcher(testString) : Boolean);
		}
		...
		this.buildContext = await esbuild.context({
			...
			define: this.define,
			plugins: [
				this.external && this.local && local(this.local),
