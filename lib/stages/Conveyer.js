...
import { setNodePath } from "#utils/nodePath";
...
export class Conveyer {
	constructor(stages, options = {}) {
		...
		for (const stage of stages.flat(stagesFlatDepth))
			if (stage) {
				...
				stage.conveyer = this;
				stage.conveyerContext = this.context;
	...
	context = {
		conveyerVersion: JSON.parse(readFileSync(path.join(Packages.getClosestPackageDir(new URL(import.meta.url).pathname), "package.json"))).version
	};
	...
	#handlePackageJSONChange = async () => {
		...
		if (this.#packagesHash !== packagesHash) {
			this.#packagesHash = packagesHash;
			this.context.packages = new Packages({
				external: this.options.externalPackages,
				local: this.options.localPackages
			});
		}
