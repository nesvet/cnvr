			...
			if (metafile) {
				...
				for (const packagePath of packagePaths)
					if (!sources.includes(packagePath)) {
						const isLocal = localModuleRegExp?.test(packagePath);
						if (isLocal || !nodeModulesRegExp.test(packagePath.replace(basePartRegExp, "")))
							try {
								...
								packageMap.push([ packagePath, {
									...
									version,
									path: packagePath,
									isLocal: localModules?.includes(name) || (isLocal && !externalModules?.includes(name))
								} ]);
							} catch (error) {
								console.error(error);
							}
					...
					for (const source of sources) {
						...
						if (resolvedPackagePath) {
							...
							packageMap.push([ packagePath, {
								...
								path: packagePath,
								isLocal: localModules?.includes(name) || !external
							} ]);
						}
					}
						...
						if (!names.includes(name))
							for (const source of sources) {
								...
								if (resolvedPackagePath) {
									...
									const isLocal = localModules?.includes(name) || localPackageRegExp.test(value);
									const packagePath = path.dirname(resolvedPackagePath);
