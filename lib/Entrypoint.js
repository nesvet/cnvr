...
import { validExtensions } from "./Entrypoints.js";
...
export class Entrypoint {
	constructor(conveyerFileName, entrypoints, isTarget = false) {
		...
		this.conveyerName = path.basename(this.conveyerFileName).replace(/\.conveyer\.[mc]?js$/, "").replace(/^\./, "");
	...
	init() {
		...
		if (!this.conveyerProcess)
			return new Promise(resolve => {
				...
				this.conveyerProcess = childProcess.fork(this.conveyerFileName, this.entrypoints.argv, {
	...
	static {
		...
		function PackageWatcher(pkg, {
			...
			if (!watcher) {
				watcher = watch(path.join(pkg.path, "**"), {
					ignored: [
						...defaultIgnored,
						...chokidarIgnored
					].map(ignoredPath => path.isAbsolute(ignoredPath) ? ignoredPath : path.resolve(pkg.path, ignoredPath)),
					ignoreInitial: true,
					...restChokidarOptions
				});
